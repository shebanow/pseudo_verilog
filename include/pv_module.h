...
public:
    ...
    virtual ~Module()
        { if (parent_module != NULL) const_cast<Module*>(parent_module)->remove_module_instance(this); }
    ...
    virtual const uint32_t get_clock() const { return root_instance ? root_instance->get_clock() : 0u; }
...
protected:
    ...
    void add_module_instance(const Module* m) { (void) module_list.insert(module_list.end(), m); }
    void add_wire_instance(const WireBase* w) { (void) wire_list.insert(wire_list.end(), w); }
    void add_register_instance(const RegisterBase* r) { (void) register_list.insert(register_list.end(), r); }

    // Methods to remove submodules, wires, and register instances from the module
    void remove_module_instance(const Module* m) { module_list.erase(m); }
